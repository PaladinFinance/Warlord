...
contract Redeemer is IWarRedeemModule, ReentrancyGuard, Pausable, Owner {
    ...
    function joinQueue(address[] calldata tokens, uint256[] calldata weights, uint256 amount) external nonReentrant whenNotPaused {
    ...
    function _joinQueue(address token, address user, uint256 amount) internal {
        ...
        userRedeems[user].push(RedeemTicket({
            token: token,
            amount: amount,
            redeemIndex: newQueueIndex,
            redeemed: false
        }));
